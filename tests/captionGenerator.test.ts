...
describe('caption generator api', () => {
  ...
  it('generateCaptions includes hashtags and mention details', async () => {
    ...
    await generateCaptions(8, options as any)
    expect(fetchMock).toHaveBeenCalledWith(
      expect.stringContaining('/api/caption-generator/generate'),
      expect.objectContaining({
        method: 'POST',
        body: JSON.stringify({ mediaId: 8, options, platforms: ['instagram','youtube'] })
      })
    )
  })
